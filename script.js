    ...
    window.onload = function () {
      ...
      // Attach Google Sign-In to your existing button
      const googleButton = document.querySelector('.button-google');
      googleButton.addEventListener('click', () => {
        google.accounts.id.prompt(); // Trigger Google Sign-In
      });
    };
    ...
    const validationConfig = {
      ...
      password: {
        ...
        message:
          "Password must be atleast 6 characters with an uppercase, lowercase and number.",
      },
    };


    // OTP Modal
    function validateAndShowOtpModal() {
      ...
      if (!phoneRegex.test(phoneNumber)) {
        phoneError.textContent = "Invalid phone number format. Please use +91 followed by 10 digits (e.g., +911234567890).";
        return;
      }
    
      // If validation passes, clear the error and show the OTP modal
      phoneError.textContent = "";
      showOtpModal(); // Call your existing showOtpModal() function
    }
    ...
    passwordInput.addEventListener("input", function () {
      ...
      document.addEventListener("DOMContentLoaded", function () {
        ...
        let passwordsMatch = false; // Track if passwords match
        ...
        function checkPasswordMatch() {
          ...
          if (password === confirmPassword && password !== "") {
            passwordMatchIcon.style.display = "inline"; // Show green tick
            passwordsMatch = true; // Set flag to true
            alert("Passwords match!"); // Optional: Show a success message
          } else {
            passwordMatchIcon.style.display = "none"; // Hide green tick
            passwordsMatch = false; // Set flag to false
            alert("Passwords do not match. Please check and try again."); // Show error message
          }
        }
      
        // Add click event to the "Check Password" button
        checkPasswordButton.addEventListener("click", checkPasswordMatch);
      
        // Prevent form submission if passwords don't match
        form.addEventListener("submit", function (e) {
          if (!passwordsMatch) {
            e.preventDefault(); // Stop form submission
            alert("Please click the 'Check Password' button and ensure the passwords match before submitting.");
          }
        });
  ...
  if (!checkPasswordMatch()) {
    isValid = false; // Prevent form submission if passwords don't match
  }
